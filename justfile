...
functional: (clean '--quiet') (package '--quiet')
    ...
    ! unzip -l dist/echobase_linux-*-py3-none-any.whl | grep tests
    ! tar tfz dist/echobase_linux-*.tar.gz | grep tests
    # Python package should contain Core module
    tar tfz dist/echobase_linux-*.tar.gz | grep -q core
    unzip -l dist/echobase_linux-*-py3-none-any.whl | grep -q core
