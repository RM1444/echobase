...
sys.modules["openwakeword.vad"] = MagicMock()
sys.modules["faster_whisper"] = MagicMock()

from EchoBase.core import config, phrases  # noqa: E402
from EchoBase.core.main import EchoBase  # noqa: E402


class TestEchoBaseInit:
    """Tests for EchoBase initialization."""

    def test_init(self):
        """Test that EchoBase initializes with correct default values."""
        easy = EchoBase()
...
class TestEchoBaseUtilities:
    """Tests for EchoBase utility methods."""
...
class TestEchoBasePlugins:
    """Tests for EchoBase plugin management."""
    ...
    def test_load_plugins_invalid_plugin(self, mock_syspath, mock_import, capsys):
        ...
        assert "invalid_plugin.py" in captured.out
    ...
    def test_load_plugins_import_error(self, mock_syspath, mock_import, capsys):
        ...
        assert "broken_plugin.py" in captured.out
...
class TestEchoBaseRouteCommand:
    """Tests for EchoBase command routing."""
    ...
    def test_route_command_no_matching_plugin(self, mock_speak, mock_plugin_no_handle):
        ...
        assert result is True
        # "Voice UX" refactor: the reply is now a randomly chosen phrase spoken
        # without a friendly lead-in, instead of a single fixed string.
        mock_speak.assert_called_once()
        (spoken,), kwargs = mock_speak.call_args
        assert spoken in phrases._NOT_UNDERSTOOD
        assert kwargs == {"allow_lead": False}
...
class TestEchoBaseAudio:
    """Tests for EchoBase audio methods."""
    ...
    # NOTE: the old energy-based ``is_silence`` was removed in the VAD refactor;
    # endpointing now goes through the Silero VAD via ``_is_speech``. Its tests
    # were dropped with the method.

    def test_record_until_silence(self):
        """Test record_until_silence method."""
        easy = EchoBase()

        # Pin normal-pace endpointing so the test doesn't depend on the saved
        # speech_pace (a "slow" profile waits longer and would exhaust the mock).
        easy.vad_silence_hang, easy.vad_max_seconds = config.PACE_TIMING["normal"]
...
class TestEchoBaseRun:
    """Tests for EchoBase run method."""

    @pytest.fixture(autouse=True)
    def _isolate_config(self):
        """Pin a deterministic config for run() tests.

        ``EchoBase()`` loads the developer's real saved config. A custom
        assistant name (e.g. "Slave") has no pretrained wake model, so wake
        detection falls back to Whisper transcription matching — which, against
        a stream mock that returns a fixed frame forever, never matches and
        spins the ``while True`` loop indefinitely. Pinning name "Jarvis" keeps
        wake detection on the mockable openwakeword branch; ``oobe_completed``
        skips the interactive setup wizard; the no-op ``apply_voice`` avoids a
        network fetch of the voice model.
        """
        cfg = dict(config.DEFAULTS)
        cfg["oobe_completed"] = True
        with patch("EchoBase.core.main.config.load", return_value=cfg), patch.object(
            EchoBase, "apply_voice"
        ):
            yield

    @patch("EchoBase.core.main.WakeWordModel")
    @patch("EchoBase.core.main.WhisperModel")
    @patch.object(EchoBase, "load_plugins")
    ...
    @patch("subprocess.run")
    @patch("EchoBase.core.main.pyaudio")
    @patch("EchoBase.core.main.WakeWordModel")
    @patch("EchoBase.core.main.WhisperModel")
    @patch.object(EchoBase, "load_plugins")
    ...
    @patch("EchoBase.core.main.WhisperModel")
    @patch.object(EchoBase, "load_plugins")
    @patch.object(EchoBase, "wait_for_speech")
    @patch.object(EchoBase, "record_until_silence")
    @patch.object(EchoBase, "transcribe")
    @patch.object(EchoBase, "route_command")
    @patch.object(EchoBase, "flush_stream")
