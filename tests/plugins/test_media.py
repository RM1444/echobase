...
import pytest
from EchoBase.plugins import media
...
@patch.object(media, "media_control", return_value=True)
def test_handle_play_pause_toggles(mock_media_control, mock_core):
    """When handle receives 'play pause' then it sends the MPRIS toggle method."""
    result = media.handle("play pause", mock_core)

    assert result is True
    assert mock_media_control.call_args.args == ("playpause", mock_core)
    assert mock_core.speak.call_args.args[0] == "Toggled."
